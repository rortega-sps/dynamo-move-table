# Source https://github.com/shivalkarrahul/DevOps/blob/master/aws/python/aws-copy-dynamo-db-table/copy-dynamodb-table.py
...
source_session = boto3.Session(region_name='us-east-1', aws_access_key_id=source_aws_access_key_id, aws_secret_access_key=source_aws_secret_access_key)
source_dynamo_client = source_session.client('dynamodb')

target_session = boto3.Session(region_name='us-east-1', aws_access_key_id=destination_aws_access_key_id, aws_secret_access_key=destination_aws_secret_access_key)
target_dynamodb = target_session.resource('dynamodb')


dynamoclient = boto3.client('dynamodb', region_name='us-east-1', #Specify the region here
...
dynamotargetclient = boto3.client('dynamodb', region_name='us-east-1', #Specify the region here
...
def createDestinationTable(sourceTable):
    ...
    if source_table.local_secondary_indexes:
      ...
      attributeDefinitionsJustLSI = [att for att in source_table.attribute_definitions if att['AttributeName'] in justLSI if att['AttributeName'] not in justKeys]      
    ...
    if source_table.global_secondary_indexes:
      ...
      attributeDefinitionsJustGSI = [att for att in source_table.attribute_definitions if (att['AttributeName'] in justGSI) if (att['AttributeName'] not in justKeys)]
